...
app.use(cors({
    origin: 'https://my-book-spes.vercel.app/'
}));
...
app.post('/books', async (req, res) => {
    try {
        const { title, author, genre, type, pages, price, cover_photo } = req.body;
        if (!title || !author || !genre || !type || !pages || !price || !cover_photo) {
            return res.status(400).json({ error: 'All fields except publication are required.' });
        }
...
app.put('/books/:id/deactivate', async (req, res) => {
    try {
        const updatedBook = await Book.findByIdAndUpdate(req.params.id, { is_active: false }, { new: true });
